...
function selectedEditorBlocks() {
  ...
  const blocks = allBlocks.filter((block) => {
    if (!selectionIsInEditor({ commonAncestorContainer: block })) return false;
    if (range.collapsed) return block.contains(range.startContainer) || block === range.startContainer;
  ...
  return block && elements.body.contains(block) ? [block] : [];
}
