  ...
  function sanitizeElement(element) {
    ...
    if (tag === "a") {
      if (safeHref && isSafeUrl(safeHref)) {
        ...
        if (isNoteLink(safeHref)) {
          element.setAttribute("class", "note-link");
          element.setAttribute("data-note-link", noteLinkId(safeHref));
        } else {
...
function bindEvents() {
  ...
  elements.body.addEventListener("click", (event) => {
    const link = event.target.closest("a[href^='note:'], a.note-link");
    if (link) {
      event.preventDefault();
      const href = link.getAttribute("href") || "";
      const noteId = Number(link.dataset.noteLink) || noteLinkId(href);
      if (!noteId) return;
